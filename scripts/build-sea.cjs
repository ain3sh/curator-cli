#!/usr/bin/env node
/**
 * SEA Build Script for Curator
 * Creates single executable applications for multiple platforms
 *
 * Usage:
 *   node scripts/build-sea.cjs              # Build all platforms (CI)
 *   node scripts/build-sea.cjs --current-only  # Build current platform only (dev)
 */
...
console.log('🎨 Curator SEA Builder\n');
...
for (const { os, arch: targetArch, ext } of targetPlatforms) {
  const name = `curate-${os}-${targetArch}${ext}`;
...
console.log('💡 Test with: ./dist/binaries/curate-' + platform() + '-' + (arch() === 'arm64' ? 'arm64' : 'x64') + (platform() === 'win32' ? '.exe' : ''));
console.log('━'.repeat(50));
